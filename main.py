...
def plot_ph_diagram(points):
    pressures = np.logspace(5, np.log10(250e6), 100)  # From 0.1 to 100 MPa
    h_f = []
    h_g = []

    critical_pressure = PropsSI('Pcrit', 'Water')
    for P in pressures:
        if P < critical_pressure:
            h_f.append(PropsSI('H', 'P', P, 'Q', 0, 'Water'))
            h_g.append(PropsSI('H', 'P', P, 'Q', 1, 'Water'))
        else:
            h_f.append(PropsSI('H', 'P', critical_pressure, 'Q', 0, 'Water'))
            h_g.append(PropsSI('H', 'P', critical_pressure, 'Q', 1, 'Water'))
    ...
    for T in temperatures:
        h = [PropsSI('H', 'P', P, 'T', T, 'Water') for P in pressures]
    ...
    p_values = []
    for point in points:
        h = PropsSI('H', 'P', point['P'], 'T', point['T'], 'Water')
        h_values.append(h)
    ...
    plt.title('p-h Diagram for Water up to 100 MPa')
...
st.title("Interactive p-h Diagram for Water")
